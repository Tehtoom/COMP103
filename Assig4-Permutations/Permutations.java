...
public class Permutations {
    ...
    public void extendPermutation(Set<String> remainingItems, Stack<String> permutationSoFar, List<List<String>> allPermutations){
        if(remainingItems.isEmpty()){
            ...
            if(counter < 10000){  
                allPermutations.add(new ArrayList<>(permutationSoFar));  // add copy
            }
            counter++;
            return;
        }

        for (String item : new HashSet<>(remainingItems)) { // copy to avoid modifying
            remainingItems.remove(item);
            permutationSoFar.push(item);

            extendPermutation(remainingItems, permutationSoFar, allPermutations);

            permutationSoFar.pop();       // backtrack
            remainingItems.add(item);     // restore item
        }
    }
